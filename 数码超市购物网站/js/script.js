window.onload = function () {
    ...
    selectedViewList.onclick = function (e) {
	  ...
	  for (var i = 0; i < tr.length; i++) {
        ...
        tr[i].getElementsByTagName('input')[1].onkeyup = function () {
            ...
            subTotal(this.parentNode.parentNode); //更新小计
            getTotal(); //更新总数
        }
    }
	//计算每种商品的金额
    function subTotal(tr) {
        ...
        var countInput = parseInt(tr.getElementsByTagName('input')[1].value); //数目input
        var reduce = tr.getElementsByTagName('span')[1]; //-号
        //写入HTML
        subtotal.innerHTML = (countInput * price).toFixed(2);
        //如果数目只有一个，把-号去掉
